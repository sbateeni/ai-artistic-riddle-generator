...
interface RiddleDisplayProps {
  riddle: ArtisticRiddle;
  onStartImageGeneration: (riddle: ArtisticRiddle) => void;
  onImproveDescription: (riddle: ArtisticRiddle) => void;
  isImproving: boolean;
}

const RiddleDisplay: React.FC<RiddleDisplayProps> = ({ riddle, onStartImageGeneration, onImproveDescription, isImproving }) => {
  const [showSolution, setShowSolution] = useState(false);
  const isEnglish = /[a-zA-Z]/.test(riddle.title);
    ...
    <div dir={dir} className="bg-gray-800/40 border border-gray-700 rounded-xl shadow-lg overflow-hidden animate-fade-in">
      <div className="p-6 bg-gray-800 flex justify-between items-center">
        <h3 className="text-2xl font-bold">
          <span className="text-purple-400 mr-3 rtl:mr-0 rtl:ml-3">{isEnglish ? `Riddle #${riddle.riddleNumber}:` : `اللغز #${riddle.riddleNumber}:`}</span>
          {riddle.title}
        </h3>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="border-r-4 border-purple-500 pr-4 rtl:border-r-0 rtl:border-l-4 rtl:pr-0 rtl:pl-4">
          <div className="flex justify-between items-center mb-2">
              ...
              <button
                  onClick={() => onImproveDescription(riddle)}
          ...
          <p className="text-gray-300 whitespace-pre-wrap leading-relaxed">{riddle.description}</p>
        </div>
        ...
        <div>
          ...
          {showSolution && (
             <div className="border-r-4 border-green-500 pr-4 rtl:border-r-0 rtl:border-l-4 rtl:pr-0 rtl:pl-4 animate-fade-in">
                <p className="text-gray-300 whitespace-pre-wrap leading-relaxed bg-gray-900/50 p-3 rounded-md">{riddle.solution}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 bg-gray-800/50 border-t border-gray-700">
        <button 
          onClick={() => onStartImageGeneration(riddle)}
